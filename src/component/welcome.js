...
import { settopbar, setimagedisplay } from '../store/common'
...
const Welcome = () => {
  ...
  const routeChat = () => {
    ...
    Service({
      ...
      if (res.data.code === '0000') {
        ...
        console.log(token);
        navigate('/livechat', { state: { room: res.data.data, token: token } })
        setLoading(false)
      }
    })
  ...
  const fetchData = () => {
    Service({
      ...
      const indexmsg = getmsg.findIndex(dummy => dummy.status === 'WELCOME')
  ...
  useEffect(() => {
    ...
    if (window.location !== window.parent.location) {
      ...
      // window.location.href = "https://app.goochat.net"
    }
    if (token && localStorage.getItem('room')) {
      navigate('/livechat', { state: JSON.parse(localStorage.getItem('room')), token: token })
    }
    fetchData()
    fetch('https://ipapi.co/json/').then(
      ...
      setDataforcreate((prevState) => ({
        ipAddress: converttojson.ip,
        region: converttojson.timezone.split('/')[0],
        country: converttojson.country_name,
        city: converttojson.country_capital,
      ...
      <>
          ...
          <div style={{ height: '153px', background: '#2A2A54', position: 'relative' }}>
            <div style={{ height: '125px', background: 'white', width: '266px', position: 'absolute', bottom: '-30px', marginLeft: '27px', borderRadius: '5px' }}>
              <div style={{ width: '100%', height: '100%', position: 'relative' }}>
                ...
                <div style={{ position: 'absolute', bottom: '24px', padding: '0 24px', textAlign: 'center' }}>
                  {/* <span style={{ fontSize: '20px', fontWeight: '600' }}>Please Click to Start Chat to talk to us.</span> */}
                  <span style={{ fontSize: '20px', fontWeight: '600' }}>Welcome to our LiveChat!.</span>
                </div>
              </div>
            </div>
          </div>
          <div style={{
            ...
            <div style={{ maxHeight: '265px', overflowY: 'auto', paddingBottom: '30px' }}>
              ...
              <div style={{ display: 'flex' }}>
                <div style={{ width: '20px', height: '20px', marginRight: '3px', backgroundImage: `url(${setting.avatars.source})`, backgroundSize: 'cover', borderRadius: '3px' }}>
        ...
        <div className="footer">
          <div style={{ textAlign: 'center', marginBottom: '10.5px' }}>
            <button style={{ height: '36px', background: '#5353C9', width: '210px', color: '#fff', fontSize: '20px', border: '0', borderRadius: '6px' }}
              onClick={routeChat}
          ...
          <div style={{ height: '36px', fontSize: '13px', fontWeight: '400', textAlign: 'center' }}>
            <span >By continuing as a guest, you agree to <br />
              <a href="htts://app.goochat.net">GooChat Terms</a>, <a href="htts://app.goochat.net">Privacy Policy</a> and <a href="htts://app.goochat.net">Cookies Policy</a>.</span>
          </div>
